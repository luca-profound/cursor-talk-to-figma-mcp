...
function formatWebSocketError(error: unknown): string {
  ...
  if (typeof error === 'object' && error) {
    ...
    return JSON.stringify({ type, message, data, raw: event }, (_key, value) => {
...
function parseServerConfig(rawValue?: string) {
  ...
  if (hasScheme) {
    try {
      ...
      const host = parsed.host; // includes port when provided
  ...
  return {
    baseUrl: hasExplicitPort
      ? `${wsProtocol}://${hostPart}:${port}`
      : `${wsProtocol}://${hostPart}`,
